...
public class UranusRenderProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, ResourceKey<Level> dimension) {
		...
		if (dimension == (ResourceKey.create(Registries.DIMENSION, new ResourceLocation("adastraextra:uranus")))) {
			...
			renderSun(15, (float) (world.getTimeOfDay(Minecraft.getInstance().getPartialTick()) * 360.0F), (int) (255 << 24 | 255 << 16 | 255 << 8 | 255), false);
			{
				ResourceLocation _texturelocation = new ResourceLocation(("adastraextra" + ":textures/" + "sky/uranusringskybox" + ".png"));
				RenderSystem.setShaderTexture(0, _texturelocation);
				Minecraft.getInstance().getTextureManager().bindForSetup(_texturelocation);
			}
