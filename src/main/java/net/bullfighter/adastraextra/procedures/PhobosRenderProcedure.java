...
public class PhobosRenderProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, ResourceKey<Level> dimension) {
		if (dimension == null)
			return;
		if (dimension == (ResourceKey.create(Registries.DIMENSION, new ResourceLocation("adastraextra:phobos")))) {
			...
			{
				ResourceLocation _texturelocation = new ResourceLocation(("ad_astra" + ":textures/" + "sky/mars" + ".png"));
				RenderSystem.setShaderTexture(0, _texturelocation);
				Minecraft.getInstance().getTextureManager().bindForSetup(_texturelocation);
			}
			renderTexture(9, 0, -60, 0, (int) (255 << 24 | 255 << 16 | 255 << 8 | 255), false);
		}
	}
}
